...
The CLAMM uses **logarithmic ticks**: a tick is a fixed *fraction* of the price, so the tick width scales with magnitude and stays reasonable everywhere (≈100 near price 1000, ≈1e-6 near price 0.001). **`precision` controls how wide the tick is** — lower precision = wider ticks, higher precision = finer ticks.

**The tick math is owned by the shared npm package `biatec-concentrated-liquidity-amm`** (repo `../BiatecCLAMM`, `src/ticks/`) so the frontend and every integrator snap to the same ticks. Prefer the package's number-based exports; do NOT fork the math into the frontend. Key exports: `TICK_TYPES`/`TickType` (`'wide'|'normal'|'narrow'`), `precisionForTickType`/`tickTypeForPrecision` (code↔number map: wide=1, normal=2, narrow=3), `getTickSize`/`cleanLogTick` (clean 1/2/5×10^k tick for a price), `tickDecimals`, `snapPriceToTick`, plus primitives `initPriceDecimals`/`priceTickDecimals`. Cross-instance note: the package bundles its own `bignumber.js`, so only consume its **number-returning** functions in the frontend (its `initPriceDecimals` returns a foreign BigNumber). To change the ticks, edit `../BiatecCLAMM/src/ticks/`, run its `test:ticks`, `npm run build-package`, publish, then bump the dep here.
...
- **`scripts/asset/initPriceDecimals.ts`** — local BigNumber copy (mirrors the package; kept local to avoid the cross-instance BigNumber hazard) used by `calculateDistribution`.
- **`scripts/asset/calculateDistribution.ts`** — walks `initPriceDecimals` and **dedups overlaps** into the clean, log-spaced `min[]/max[]` grid (e.g. `0.90→1.00` in one cell) for the slider/chart.
- **`components/LiquidityComponents/AddLiquidity.vue`** — price range usable via **both** number inputs and slider. Keep: InputNumber `:step` = `cleanLogTick(price, precision)` (from the package, window-independent, correct at any magnitude); input decimals = `tickDecimals(step)`; `ensureWindowCoversTrade()` grows the grid window to span the chosen range; typed values snap to the nearest grid boundary via `snapMin/MaxPriceToGrid` and **clamp at the first/last tick**; tick width is chosen as a localized **tick type** (`selectTickType`/`currentTickType`, labels under `components.addLiquidity.tickTypes.*` in all 10 locales) that maps to `state.precision`; pool bounds from the route query pin exactly through `activeRouteRange`.
